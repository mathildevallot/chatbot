# -*- coding: utf-8 -*-
"""documents_plui.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1vSWVJK0F70s60oCEi2qZz0hmGswkNHDH
...
def chunk_text(text_per_page, chunk_size=500, chunk_overlap=100):
    chunks = []
    for page_data in tqdm(text_per_page, desc="Découpage du texte en chunks"):
        ...
        while start_index < len(page_text):
            ...
            # Assurer que le dernier chunk inclut la fin du texte même s'il est plus petit que chunk_size
            if start_index >= len(page_text) and len(page_text) > 0 and (len(page_text) - (start_index - (chunk_size - chunk_overlap))) < chunk_size:
                # Si le dernier chunk était trop petit et qu'il reste du texte, nous ajoutons le reste.
                # Ou si le précédent chunk a déjà couvert la fin, nous nous assurons de ne pas ajouter de duplicata.
                pass
            elif end_index >= len(page_text) and start_index < len(page_text):
                break # Sortir si on a déjà couvert la fin

    # Nettoyage des chunks vides et suppression des duplicatas si le chevauchement a créé des chunks identiques à la fin.
    final_chunks = []
    seen_chunks = set()
    for chunk_data in chunks:
        if chunk_data['chunk_text'] and chunk_data['chunk_text'] not in seen_chunks:
            final_chunks.append(chunk_data)
            seen_chunks.add(chunk_data['chunk_text'])

    return final_chunks

# Définir la taille des chunks et le chevauchement
CHUNK_SIZE = 500  # Nombre de caractères par chunk
CHUNK_OVERLAP = 100 # Nombre de caractères de chevauchement entre les chunks

text_chunks = chunk_text(all_pages_text, chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP)

print(f"Nombre total de chunks créés : {len(text_chunks)}")

# Afficher un aperçu des premiers chunks
if text_chunks:
